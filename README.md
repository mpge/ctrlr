...
Every `ctrlr start` opens with an ASCII lockup of the controller + wordmark, gradient-tinted from `#b14bff` (pink) to `#3ed8ff` (cyan), with a spinner under it while the engine boots:
...
C O N T R O L   Y O U R   A I   A G E N T S
◐ Booting engine · press Enter to skip
```

Press <kbd>Enter</kbd> to skip ahead — otherwise it dwells for 900 ms so the engine has time to spawn every pane before the grid mounts.
